...
import { addItem, selectCartItems } from '../store/cartSlice'; // Import addItem and selector
...
function ProductDetail() {
  ...
  // ... (useEffect for fetching product remains the same)
  ...
  const handleAddToCart = () => {
    if (product) {
      ...
      if (existingItem) {
          ...
          alert(`${product.name} is already in your cart. You can adjust quantity in the cart page.`);
      } else {
  ...
  return (
    <div class='container mx-auto p-4 flex flex-col md:flex-row items-center'>
      <img src={product.imageUrl || 'https://via.placeholder.com/500x400'} alt={product.name} class='w-full md:w-1/2 h-auto object-cover rounded-lg shadow-md mb-6 md:mb-0 md:mr-8' />
      <div class='md:w-1/2'>
        <h2 class='text-3xl font-bold mb-4'>{product.name}</h2>
        <p class='text-xl text-green-600 font-bold mb-6'>${product.price.toFixed(2)}</p>
